...
def main():
    model_dir = ROOT/'models/deep_bigru'
    ...
    videos = {}
    for path in sorted((ROOT/'data/videos').rglob('*.mp4')):
        assert path.stem not in videos, f'Duplicate source video: {path.stem}'
        videos[path.stem] = path
    records = []
    for path in sorted((ROOT/'data/features').glob('*_features.csv')):
        ...
        keypoints = ROOT/'data/keypoints/normalized'/f'{video_id}_keypoints.csv'
        kd = pd.read_csv(keypoints)
        assert kd.frame_no.is_unique and np.array_equal(kd.frame_no, df.frame_no), video_id
        raw = ROOT/'data/keypoints/raw'/f'{video_id}_raw_batsman_keypoints.csv'
        if raw.exists():
        ...
        cap.release()
        label = ROOT/'data/labels'/f'{video_id}_phase_labels.csv'
        if label.exists():
    ...
    samples, features, skipped = build_deep_bigru_samples(ROOT/'data/features', ROOT/'data/labels')
    ...
    pd.DataFrame(records).to_csv(ROOT/'reports/dataset_manifest.csv', index=False)
    ...
    pd.DataFrame(extra).to_csv(ROOT/'reports/additional_videos.csv', index=False)
    ...
    (ROOT/'reports/alignment_report.json').write_text(json.dumps(report, indent=2)+'\n')
    hashes = []
    ...
    (ROOT/'reports/checksums.sha256').write_text('\n'.join(hashes)+'\n')
